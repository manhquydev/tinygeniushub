...
rows=$(docker exec root-postgres-1 psql -U postgres -d cungcontuhoc -At -F '|' -c "SELECT c.slug, l.id, cl.\"orderNo\" FROM \"Course\" c JOIN \"CourseLesson\" cl ON cl.\"courseId\"=c.id JOIN \"Lesson\" l ON l.id=cl.\"lessonId\" WHERE c.\"isPublished\"=true AND cl.\"orderNo\"<=3 ORDER BY c.slug, cl.\"orderNo\";")
