...
import { markLessonVideoWatched } from "@/modules/learning/video-watch-service";
...
export async function POST(
  ...
  try {
    ...
    const ipLimit = await enforceRateLimit({
      key: `learning:watch:complete:ip:${ip}`,
      limit: ipPolicy.limit,
      windowMs: ipPolicy.windowMs,
      storeFailureMode: "deny",
    });
    if (!ipLimit.allowed) {
      return fail("Too many watch completion requests. Please retry later.", 429, {
        retryAfterMs: ipLimit.retryAfterMs,
      });
    }

    const parentLimit = await enforceRateLimit({
      key: `learning:watch:complete:parent:${buildRateLimitIdentity(parent.id)}`,
    ...
    const watch = await markLessonVideoWatched({
      parentId: parent.id,
      lessonId,
      payload,
    });
