...
describe("blog newsletter verify route", () => {
  ...
  it("redirects to subscribed=false when token is invalid", async () => {
    ...
    expect(response.headers.get("location")).toBe("https://www.tinygeniushubvn.tech/blog?subscribed=false");
  });
  ...
  it("redirects to subscribed=true when token is valid", async () => {
    ...
    expect(response.status).toBe(307);
    expect(response.headers.get("location")).toBe("https://www.tinygeniushubvn.tech/blog?subscribed=true");
  });
});
