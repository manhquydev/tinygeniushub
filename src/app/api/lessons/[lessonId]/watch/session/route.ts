...
import { createLessonVideoWatchSession } from "@/modules/learning/video-watch-service";
...
export async function POST(
  ...
  try {
    ...
    const [ipPolicy, parentPolicy] = await Promise.all([
      getRateLimitPolicy("learning.watch.session.ip"),
      getRateLimitPolicy("learning.watch.session.parent"),
    ]);
    const ip = getRequestIp(request);
    const ipLimit = await enforceRateLimit({
      key: `learning:watch:session:ip:${ip}`,
      limit: ipPolicy.limit,
      windowMs: ipPolicy.windowMs,
      storeFailureMode: "deny",
    });
    if (!ipLimit.allowed) {
      return fail("Too many watch session requests. Please retry later.", 429, {
        retryAfterMs: ipLimit.retryAfterMs,
      });
    }

    const parentLimit = await enforceRateLimit({
      key: `learning:watch:session:parent:${buildRateLimitIdentity(parent.id)}`,
