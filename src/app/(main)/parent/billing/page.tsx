...
export default async function ParentBillingPage() {
  ...
  const latestSuccessfulPayment = succeededPayments[0] ?? null;
  ...
  return (
    <div className="page-stack">
      ...
      <section className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        ...
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <h2 className="text-base font-extrabold text-slate-900 sm:text-lg">Thông tin thanh toán</h2>
          <div className="mt-4 grid gap-3">
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs uppercase tracking-[0.08em] text-slate-500">Mô hình sử dụng</p>
              <p className="mt-1 text-sm font-bold text-slate-900">Mua theo từng khóa học</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs uppercase tracking-[0.08em] text-slate-500">Kênh thanh toán chính</p>
              <p className="mt-1 text-sm font-bold text-slate-900">PayOS (chuyển khoản ngân hàng)</p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs uppercase tracking-[0.08em] text-slate-500">Giao dịch thành công gần nhất</p>
              <p className="mt-1 text-sm font-bold text-slate-900">
                {latestSuccessfulPayment ? `${formatDate(latestSuccessfulPayment.processedAt)} • ${formatCurrency(latestSuccessfulPayment.amountVnd)}` : "Chưa có giao dịch thành công"}
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
              <p className="text-xs uppercase tracking-[0.08em] text-slate-500">Tổng giao dịch đã ghi nhận</p>
              <p className="mt-1 text-sm font-bold text-slate-900">{payments.length} giao dịch</p>
            </div>
          </div>
        </article>
      </section>
