...
export function ActivityPanel({
  ...
  const mascotMessage =
    result === "correct"
      ? t("activity.mascotCorrect")
      : result === "wrong"
        ? t("activity.mascotWrong")
        : activityLoading
          ? t("activity.mascotPreparing")
          : t("activity.mascotChoose");
  ...
  return (
    <div className="lp-main">
      <div
        ...
        aria-label={t("sentenceProgress", { current, total: totalActivities })}
      >
      ...
      >
        {activityLoading && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", padding: "1.5rem 0", color: "var(--lp-ink-muted)" }}>
            ...
            <span style={{ fontSize: "0.82rem", fontWeight: 600 }}>{t("activity.loading")}</span>
          </div>
        )}
          ...
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.65rem" }}>
              <span style={{ fontSize: "0.68rem", fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--lp-ink-muted)" }}>
                {t("activity.sentenceCounter", { current, total: totalActivities })}
              </span>
          ...
          {result !== "idle" && (
            ...
            >
              ...
              <span className="lp-feedback-text">
                {result === "correct" ? t("activity.feedbackCorrect") : t("activity.feedbackRetry")}
              </span>
            </m.div>
          )}
        </AnimatePresence>
      ...
      <AnimatePresence>
        {result === "correct" && (
          ...
          >
            <Image
              src={STICKER_CORRECT}
              alt={t("activity.stickerCorrectAlt")}
              width={110}
              height={110}
        ...
        {result === "wrong" && (
          ...
          >
            <Image
              src={STICKER_WRONG}
              alt={t("activity.stickerWrongAlt")}
              width={110}
              height={110}
              style={{ objectFit: "contain" }}
            />
