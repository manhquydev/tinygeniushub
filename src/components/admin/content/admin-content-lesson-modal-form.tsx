  ...
  return (
    <AdminContentModalShell title={props.title} open={props.open} onClose={props.onClose}>
      <form className="grid gap-3" onSubmit={(event) => void props.onSubmit(event)}>
        {props.mode === "create" ? (
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="grid gap-1.5">
              <Label htmlFor="lesson-order">Order (orderNo)</Label>
              <Input
                id="lesson-order"
          ...
          <div className="grid gap-2 rounded-lg border border-[var(--admin-card-border)] bg-[var(--admin-sidebar-accent)] p-2 text-xs text-[var(--admin-text-secondary)]">
            <p><span className="font-semibold">Order:</span> {props.editingLesson?.orderNo ?? "-"}</p>
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="lesson-objective">Target</Label>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="grid gap-1.5">
            <Label htmlFor="lesson-minutes">Number of minutes</Label>
          ...
          <div className="flex items-center gap-2 rounded-lg border border-[var(--admin-card-border)] bg-[var(--admin-sidebar-accent)] px-3 py-2">
            <Checkbox
              id="lesson-trial"
              checked={props.form.trialEnabled}
              onCheckedChange={(checked) => props.onFormChange((current) => ({ ...current, trialEnabled: !!checked }))}
            />
            <Label htmlFor="lesson-trial" className="text-sm cursor-pointer">Turn on trial lessons</Label>
          </div>
        </div>

        <div className="grid gap-1.5">
          <Label htmlFor="lesson-video">Video source</Label>
        ...
        {props.mode === "edit" && props.editingLessonId ? (
          <div className="grid gap-2 rounded-lg border border-[var(--admin-card-border)] bg-[var(--admin-sidebar-accent)] p-3">
            <p className="text-xs font-semibold text-[var(--admin-text-secondary)]">Video Bunny Stream</p>
            {props.editingLessonBunny?.bunnyVideoId ? (
              <p className="text-xs text-[var(--admin-text-secondary)]">
                ID: <code>{props.editingLessonBunny.bunnyVideoId}</code>· Status:<strong>{props.editingLessonBunny.videoStatus}</strong>
              </p>
            ) : (
              <p className="text-xs text-[var(--admin-text-muted)]">There is no Bunny video yet</p>
            )}
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="lesson-offline">Offline tag content (Markdown)</Label>
        ...
        <div className="grid gap-1.5">
          <Label htmlFor="lesson-script">Parent script (Markdown)</Label>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={props.submitting} className="bg-teal-600 hover:bg-teal-700">
            {props.submitting ? "Saving..." : props.mode === "create" ? "Create lessons" : "Save updates"}
          </Button>
        </div>
      </form>
