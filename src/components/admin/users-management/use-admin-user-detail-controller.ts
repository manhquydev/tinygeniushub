...
export function useAdminUserDetailController(input: UseAdminUserDetailControllerInput) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!response.ok || !body.ok || !body.data?.detail) { setDetailError(body.error?.message ?? "Không tải được thông tin chi tiết."); setDetail(null); return; }
        setDetail(body.data.detail);
      } catch (e) {
        if (e instanceof Error && e.name === "AbortError") return;
        setDetailError(e instanceof Error ? e.message : "Lỗi không xác định."); setDetail(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!response.ok || !body.ok) { setNotesError(body.error?.message ?? "Không tải được ghi chú nội bộ."); setNotes([]); return; }
        setNotes(body.data?.notes ?? []);
      } catch (e) {
        if (e instanceof Error && e.name === "AbortError") return;
        setNotesError(e instanceof Error ? e.message : "Lỗi không xác định."); setNotes([]);
  ...
  async function handleImpersonate(parentId: string) {
    if (!window.confirm("Bắt đầu xem dưới quyền người dùng này?")) return;
    ...
    try {
      ...
      if (!response.ok || !body.ok) { setDetailError(body.error?.message ?? "Không thể chuyển sang chế độ xem người dùng."); return; }
      const redirectTo = body.data?.redirectTo ?? "/parent/dashboard";
      input.router.push(redirectTo); input.router.refresh();
    } catch (e) { setDetailError(e instanceof Error ? e.message : "Lỗi không xác định."); }
  ...
  async function handleCreateNote() {
    ...
    if (note.length === 0) { setNotesError("Vui lòng nhập nội dung ghi chú."); return; }
    ...
    try {
      ...
      if (!response.ok || !body.ok) { setNotesError(body.error?.message ?? "Không thể thêm ghi chú."); return; }
      setNoteDraft(""); setNotesReloadToken((v) => v + 1);
    } catch (e) { setNotesError(e instanceof Error ? e.message : "Lỗi không xác định."); }
    finally { setNoteSubmitting(false); }
  }

  async function handleSubscriptionAction(action: "extend" | "cancel" | "activate") {
    if (!input.selectedParentId) return;
    if (!detail?.currentSubscription) { setSubscriptionActionFeedback("Người dùng chưa có gói đăng ký để thao tác."); return; }
    ...
    try {
      ...
      if (!response.ok || !body.ok) { setSubscriptionActionFeedback(body.error?.message ?? "Không cập nhật được subscription."); return; }
      setSubscriptionActionFeedback("Đã cập nhật subscription."); setDetailReloadToken((v) => v + 1);
    } catch (e) { setSubscriptionActionFeedback(e instanceof Error ? e.message : "Lỗi không xác định."); }
  ...
  async function handleSendManualEmail() {
    ...
    if (!subject || !bodyText) { setManualEmailFeedback("Vui lòng nhập tiêu đề và nội dung email."); return; }
    ...
    try {
      ...
      if (!response.ok || !payload.ok) { setManualEmailFeedback(payload.error?.message ?? "Không gửi được email."); return; }
      setManualEmailFeedback(`Đã gửi email thành công (${payload.data?.provider ?? "unknown"}).`);
      setManualEmailSubject(""); setManualEmailBody("");
    } catch (e) { setManualEmailFeedback(e instanceof Error ? e.message : "Lỗi không xác định."); }
