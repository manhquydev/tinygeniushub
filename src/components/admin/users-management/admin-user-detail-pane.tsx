...
function SimpleListBlock({ title, empty, children }: { title: string; empty: string; children: ReactNode }) {
  return (
    <div>
      ...
      <div>{children || <p className="text-sm text-slate-500">{empty}</p>}</div>
    </div>
  );
}
...
export function AdminUserDetailPane(props: AdminUserDetailPaneProps) {
  return (
    <aside className="rounded-xl border border-slate-200 bg-white p-4 overflow-y-auto">
      {!props.selectedParentId && (
        <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500 text-center">
      ...
      {!props.detailLoading && !props.detailError && props.detail && (
        <div className="space-y-4">
          {/* Header */}
          <div>
            <h3 className="text-base font-bold text-slate-900">{props.detail.parent.displayName ?? props.detail.parent.email}</h3>
            <p className="text-xs text-slate-500">{props.detail.parent.email}</p>
            <p className="mt-1 text-xs text-slate-500">Ngày tham gia: {formatDate(props.detail.parent.createdAt)}</p>
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
              ...
              <p className="mt-1 text-lg font-bold text-slate-800">{props.detail.parent.notificationCount}</p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-2">
              <p className="text-slate-500">Giao dịch thành công</p>
              <p className="mt-1 text-lg font-bold text-slate-800">{props.selectedUser?.successfulPaymentsCount ?? 0}</p>
            </div>
          </div>

          {/* Subscription */}
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Gói đăng ký</p>
            {props.detail.currentSubscription ? (
              ...
              <p className="text-sm text-slate-600">Chưa có gói đăng ký.</p>
            )}
            ...
            {props.subscriptionActionFeedback && <p className="text-xs text-slate-700">{props.subscriptionActionFeedback}</p>}
          </div>

          {/* Email */}
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Gửi email thủ công</p>
            ...
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs text-slate-500">{5000 - props.manualEmailBody.length} ký tự còn lại</p>
            ...
            {props.manualEmailFeedback && <p className="text-xs text-slate-700">{props.manualEmailFeedback}</p>}
          </div>

          {/* Lists */}
          <SimpleListBlock title="Lịch sử gói đăng ký" empty="Chưa có lịch sử.">
            {props.detail.subscriptionHistory.slice(0, 8).map((item) => (
              <div key={item.id} className="mb-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs">
                <p className="font-semibold">{item.planCode ?? "Gói không xác định"} - {item.status}</p>
                <p className="text-slate-500">{item.provider} / {item.providerTransactionId} - {formatDate(item.processedAt)}</p>
              </div>
            ))}
          </SimpleListBlock>
          ...
          <SimpleListBlock title="Bé (hoạt động 30 ngày)" empty="Chưa có hồ sơ bé.">
            {props.detail.children.map((child) => (
              <div key={child.id} className="mb-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-sm text-slate-700">
          ...
          <SimpleListBlock title="Lịch sử thanh toán (10 gần nhất)" empty="Chưa có giao dịch.">
            {props.detail.paymentHistory.map((payment) => (
              <div key={payment.id} className="mb-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs">
                <p className="font-semibold">{toCurrency(payment.amountVnd)} {payment.currency} - {payment.status}</p>
          ...
          <SimpleListBlock title="Lời mời người chăm sóc" empty="Chưa có lời mời.">
            {props.detail.caregiverInvites.map((invite) => (
              <div key={invite.id} className="mb-1.5 rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs">
                <p className="font-semibold">{invite.email}</p>
                <p className="text-slate-500">{invite.accepted ? "Đã chấp nhận" : "Đang chờ"} - hết hạn {formatDate(invite.expiresAt)}</p>
              </div>
            ))}
          </SimpleListBlock>
          ...
          {/* Notes */}
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Ghi chú nội bộ</p>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 space-y-2">
              <Textarea
              ...
              />
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs text-slate-500">{500 - props.noteDraft.length} ký tự còn lại</p>
            ...
            {props.notesLoading && <p className="mt-2 text-xs text-slate-500">Đang tải ghi chú...</p>}
            <div className="mt-2 space-y-1.5">
              {props.notes.map((note) => (
                <article key={note.id} className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 text-xs">
                  <p className="text-slate-700">{note.note}</p>
                  <p className="mt-1 text-slate-500">{note.createdBy} - {formatDate(note.createdAt)}</p>
                </article>
              ))}
              {!props.notesLoading && props.notes.length === 0 && <p className="text-xs text-slate-500">Chưa có ghi chú.</p>}
            </div>
          </div>
        </div>
      )}
