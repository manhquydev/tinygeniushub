...
const TREND_CONFIG = {
  IMPROVING: { icon: "↑", label: "Making progress", colorClass: "text-green-600 bg-green-50" },
  STABLE:    { icon: "→", label: "Stable",       colorClass: "text-slate-500 bg-slate-100" },
  DECLINING: { icon: "↓", label: "Need more practice", colorClass: "text-red-500 bg-red-50" },
};
...
export function SkillDetailCard({ skill, mastery, recentAttempts, nextReview, trend, prerequisites }: SkillDetailCardProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 mt-0.5">Grade {skill.gradeLevel}</p>
          </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">{pct}% proficient</p>
            <p className="text-xs text-slate-400">{mastery.totalAttempts} practice attempts · {mastery.correctAttempts} correct</p>
            {nextReview && (
      ...
      {/* Recent attempts */}
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <h2 className="font-semibold text-slate-700 text-sm mb-3">Recent activity</h2>
        {recentAttempts.length === 0 ? (
          <p className="text-sm text-slate-400 py-2">There are no exercises yet.</p>
        ) : (
      ...
      {prerequisites.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="font-semibold text-slate-700 text-sm mb-2">Prerequisite skills</h2>
